...
# Check if file is in app/ dir
if [[ "$file_path" != */app/* ]]; then
    exit 4
fi

# Find project root by looking for app/ dir
...
# If didn't find project root with both app/ and tests/ dirs, try one more time from the app dir
if [ -z "$project_root" ]; then
    if [[ "$file_path" == */app/* ]]; then
        ...
        if [ -d "$potential_root/app" ] && [ -d "$potential_root/tests" ]; then
            ...
            exit 7  # Missing tests dir
        elif [ ! -d "$potential_root/app" ] && [ -d "$potential_root/tests" ]; then
            exit 7  # Missing app dir
        else
...
# Make sure the file is actually in the app dir
...
# Helpers with integration tests
    ...
    # Some projects might have unit tests for helpers
    test_path="${tests_dir}/unit/${relative_path%.*}-test.ts"
    test_paths+=("$test_path")
    test_path="${tests_dir}/unit/${relative_path%.*}-test.js"
    test_paths+=("$test_path")

# These may have unit tests
...
# Templates with integration tests
...
# Modifiers with integration tests
...
# Any other files in lib/ or other directories
    ...
    # Try both unit and integration tests
    test_path="${tests_dir}/unit/${relative_path%.*}-test.ts"
    test_paths+=("$test_path")
    test_path="${tests_dir}/unit/${relative_path%.*}-test.js"
    test_paths+=("$test_path")
